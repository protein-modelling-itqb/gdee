...
class Measurer:
    ...
    def measure_from_files(self, protein_file, ligand_file):
        """
        Perform measurements using protein and ligand files.
        """
        protein = mda.Universe(str(protein_file))
        ligand = mda.Universe(str(ligand_file))

        for task in self.task_list:
            task.set_system(protein, ligand)
            if not task.enabled:
                print(f"Could not apply measurement '{task.name}' to the provided files.")
        ...
        for task in self.task_list:
            if task.enabled:
                results.append(task.to_container())
            task.clear()
